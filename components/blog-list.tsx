...
import { PageBlocksFeaturedReading, PostConnectionQuery, PostQuery } from "@/tina/__generated__/types"
...
export function FeaturedReading(props: PageBlocksFeaturedReading) {
  return (
    <div className="relative">
      ...
      <div data-tina-field={tinaField(props, 'featuredPost')}  className="relative -my-12 mx-auto aspect-square max-w-7xl rounded-lg py-24 shadow-lg shadow-muted sm:aspect-video sm:py-32">
        ...
        <div className="absolute bottom-5 left-5 p-6 sm:w-2/3 sm:p-12 md:w-1/2">
          <h2 data-tina-field={tinaField(props, 'label')} className="md:text-md relative mt-4 inline-block border border-white px-2 py-1 text-sm font-bold uppercase tracking-widest text-white">
            {props.label}
          </h2>
...
export function FeaturedReadingAlt({ post, hasLink }: {
  post: PostQuery['post'],
  hasLink?: boolean
}) {
  return (
    <div className="grid-rows-12 mx-auto grid grid-cols-1 overflow-hidden rounded-lg bg-card shadow-md lg:grid-cols-12 lg:grid-rows-1">
      <div className="relative col-span-6  px-4 py-8 sm:px-12 sm:py-16 lg:col-span-5 lg:min-h-[400px]">
        ...
        <p className="mt-8 text-lg leading-8 text-primary" data-tina-field={tinaField(post, 'description')}>
          {post.description}
        </p>
        {hasLink ? (
          <Link
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 flex translate-y-1/2 justify-center lg:bottom-16 lg:left-auto lg:right-0 lg:translate-x-1/2">
          <div data-tina-field={tinaField(post, 'author')} className="relative h-20 w-20 overflow-hidden rounded-full ring-4 ring-card sm:h-24 sm:w-24 md:ring-8">
            <Image
              fill={true}
              sizes="200px"
              className="object-cover"
      ...
      <div className="relative col-span-6 aspect-[4/2] lg:col-span-7 lg:aspect-auto" data-tina-field={tinaField(post, 'image')}>
        <Image
          fill={true}
          className="object-cover"
          alt=""
                    ...
                    <div className="absolute inset-x-0 bottom-0 z-10 flex translate-y-1/2 justify-center">
                      <div className="relative h-20 w-20 overflow-hidden rounded-full ring-4 ring-card md:ring-8">
                        <Image
                          ...
                          alt=""
                          src={post.author?.image || ""}
                          data-tina-field={post.author && tinaField(post.author, 'image')}
                        />
                      </div>
                    </div>
                  </div>
